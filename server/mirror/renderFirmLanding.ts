...
const STYLE = `<style id="vw-firm-landing-style">
  ...
  .vw-firm__cta{background:var(--vw-red);color:#fff;padding:clamp(5rem,8vw,7.5rem) 0}.vw-firm__cta-grid{display:grid;grid-template-columns:minmax(0,4fr) minmax(18rem,4fr);gap:12vw;align-items:end}.vw-firm__cta .vw-firm__eyebrow{color:#fff}.vw-firm__cta h2{margin:1rem 0 2rem}.vw-firm__cta p{font-size:1.05rem;line-height:1.6;max-width:28rem}.vw-firm__cta nav{border-top:1px solid rgba(255,255,255,.5)}.vw-firm__cta nav a{display:grid;grid-template-columns:2.5rem minmax(0,1fr) 2rem;gap:1rem;align-items:center;color:#fff;font-family:var(--vw-font-editorial);font-size:clamp(1.25rem,2vw,1.9rem);text-decoration:none;padding:1.35rem 0;border-bottom:1px solid rgba(255,255,255,.5)}.vw-firm__cta nav a>span{font-family:var(--vw-font-ui);color:#fff;font-size:.65rem;font-weight:600;letter-spacing:.1em}.vw-firm__cta nav b{font-family:var(--vw-font-ui);font-weight:400;transition:transform .22s ease}.vw-firm__cta nav a:hover b,.vw-firm__cta nav a:focus-visible b{transform:translateX(.35rem)}
