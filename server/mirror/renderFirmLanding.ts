...
function verifiedHistory(config: ConfigMap, key: "firm_landing_history_intro" | "firm_landing_history_body", lang: Lang, fallback: string): string {
  const candidate = cfg(config, key, lang).trim();
...
function renderHistory(config: ConfigMap, lang: Lang): string {
  const fallbackIntro = lang === "es"
    ...
    : "Founded in 1986, Von Wobeser y Sierra combines legal excellence, integrity and a multidisciplinary approach to advise clients on their most relevant matters in Mexico.";
  const fallbackBody = lang === "es"
    ? "Trabajamos como un socio estratégico, brindando asesoría preventiva y resolutiva a partir de un entendimiento profundo del negocio de cada cliente."
    : "We work as a strategic partner, providing preventive and solution-oriented counsel based on an in-depth understanding of each client’s business.";
  ...
  return `<section class="vw-firm__section vw-firm__history vw-firm-reveal" id="historia" aria-labelledby="vw-firm-history-title">
    <div class="vw-firm__container vw-firm__history-grid">
      ...
      <div class="vw-firm__history-copy">
        <div class="vw-firm__history-intro">${renderRichText(verifiedHistory(config, "firm_landing_history_intro", lang, fallbackIntro))}</div>
        <div class="vw-firm__prose">${renderRichText(verifiedHistory(config, "firm_landing_history_body", lang, fallbackBody))}</div>
      </div>
    </div>
  </section>`;
}
  ...
  return `<section class="vw-firm__section vw-firm__stats vw-firm-reveal" id="cifras" aria-labelledby="vw-firm-stats-title">
    <div class="vw-firm__container">
      ...
      <dl class="vw-firm__stats-grid">
        ${stats.map((stat) => `<div><dd>${esc(stat.value)}</dd><dt>${esc(stat.label)}</dt></div>`).join("")}
      </dl>
    </div>
  </section>`;
}
  ...
  return `<section class="vw-firm__section vw-firm__values vw-firm-reveal" id="valores" aria-labelledby="vw-firm-values-title">
    <div class="vw-firm__container vw-firm__values-grid">
      <div class="vw-firm__values-heading">
        <p class="vw-firm__eyebrow">01 — 05</p>
...
function renderFinalCta(config: ConfigMap, lang: Lang): string {
  ...
  const fallbackPaths = lang === "es"
    ? ["/attorneys", "/capacidades/practicas", "/contacto"]
    : ["/attorneys?lang=en", "/capabilities/practices", "/contact"];
  const fallbackLabels = lang === "es"
    ? ["Nuestros abogados", "Prácticas legales", "Contacto"]
    : ["Our attorneys", "Legal practices", "Contact"];
...
const STYLE = `<style id="vw-firm-landing-style">
  ...
  .vw-firm__cta{background:var(--vw-red);color:#fff;padding:clamp(5rem,8vw,7.5rem) 0}.vw-firm__cta-grid{display:grid;grid-template-columns:minmax(0,4fr) minmax(18rem,4fr);gap:12vw;align-items:end}.vw-firm__cta .vw-firm__eyebrow{color:#fff}.vw-firm__cta h2{margin:1rem 0 2rem}.vw-firm__cta p{font-size:1.05rem;line-height:1.6;max-width:28rem}.vw-firm__cta nav{border-top:1px solid rgba(255,255,255,.5)}.vw-firm__cta nav a{display:grid;grid-template-columns:2.5rem minmax(0,1fr) 2rem;gap:1rem;align-items:center;color:#fff;font-family:"Publico-Roman",Publico,Georgia,serif;font-size:clamp(1.25rem,2vw,1.9rem);text-decoration:none;padding:1.35rem 0;border-bottom:1px solid rgba(255,255,255,.5)}.vw-firm__cta nav a>span{font-family:"Geomanist-Book",Geomanist,Arial,sans-serif;color:#fff;font-size:.65rem;font-weight:600;letter-spacing:.1em}.vw-firm__cta nav b{font-family:Arial,sans-serif;font-weight:400;transition:transform .22s ease}.vw-firm__cta nav a:hover b,.vw-firm__cta nav a:focus-visible b{transform:translateX(.35rem)}
...
export function renderFirmLanding(
  ...
  const eyebrow = text(config, "firm_landing_eyebrow", lang, "Von Wobeser y Sierra");
