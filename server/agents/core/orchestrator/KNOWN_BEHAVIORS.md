...
- A Legal Council `rejected` verdict is stored as a failed article step and as an unsuccessful
  `legal_council` result, while the containing pipeline currently returns `success: true`.
- If the post-stage article has no English or Spanish body, the Legal Council emits its running
