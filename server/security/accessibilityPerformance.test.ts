...
test("la portada nombra los cuatro carruseles y aplica contraste AA al módulo de noticias", () => {
  ...
  assert.equal($("#video_header").attr("poster"), "/images/hero-20260825-dron-poster.webp");
  assert.equal($("#video_header").attr("width"), "1920");
  assert.equal($("#video_header").attr("height"), "1080");
  assert.equal($("#video_header source").eq(0).attr("data-vwb-src"), "/images/hero-20260825-dron-hd-mobile.mp4");
  assert.equal($("#video_header source").eq(0).attr("media"), "(max-width: 680px)");
  assert.equal($("#video_header source").eq(1).attr("data-vwb-src"), "/images/hero-20260825-dron-fullhd-desktop.mp4");
  ...
  assert.match($(".home__hero").attr("style") || "", /hero-20260825-dron-poster\.webp/);
  assert.equal($('link[rel="preload"][href="/images/hero-20260825-dron-poster.webp"]').attr("fetchpriority"), "high");
