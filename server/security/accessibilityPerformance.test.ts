...
test("la portada nombra los cuatro carruseles y aplica contraste AA al módulo de noticias", () => {
  ...
  assert.equal($("#video_header").attr("poster"), "/images/home-hero.webp");
  assert.equal($("#video_header source").attr("type"), "video/mp4");
  assert.match($(".home__hero").attr("style") || "", /home-hero\.webp/);
  assert.equal($('link[rel="preload"][href="/images/home-hero.webp"]').attr("fetchpriority"), "high");
...
test("la carga pública elimina librerías Joomla duplicadas y usa jQuery vigente", () => {
  ...
  assert.match(optimized, /templates\/beez3\/js\/min\/slick\.min\.js/);
});
