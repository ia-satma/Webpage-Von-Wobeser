...
El catch-all de la SPA (paso 4) DEBE ir al final: si se mueve antes de la API o del espejo, se traga esas peticiones. El espejo va después de la API y antes del catch-all para no ensombrecer `/api` ni ser ensombrecido. El orquestador de agentes arranca **dentro del callback de `listen()`** — si el puerto no bindea, no se inicializa. Las tareas de negocio no usan timers por instancia: sus comandos idempotentes están preparados para Scheduled Deployments separados, cuya activación sigue pendiente y se documenta en `docs/security/REPLIT_SCHEDULED_DEPLOYMENTS.md`.
...
| `website_auditor` | Website Auditor | **No (estructural)** | n/a | botón + comando programable | Audita enlaces/imágenes/traducciones/SEO. El comando programable fuerza `applyChanges:false`; la ejecución manual puede aplicar cambios solo si recibe autorización explícita. |
...
| `legal_alerts` | Legal Alerts | Sí | gpt-5.4-mini | botón + comando programable pendiente | Desde fuente oficial MX (.gob.mx/cofece, allowlist anti-SSRF) redacta un **borrador** bilingüe (noticia no publicada, `ready_for_approval`). El Scheduled Deployment no se activa sin aprobación por costo y efectos persistentes. |
...
- **Evidencia del build:** `npm run build` genera `dist/sbom.cdx.json` (CycloneDX) y `dist/build-provenance.json`; `npm run verify:build-evidence` recalcula sus hashes. CI conserva ambos artefactos 30 días y producción relaciona el build observado con el commit en `deployment_artifacts`.
