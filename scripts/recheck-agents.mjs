...
const run = async (agent, payload, ms = 280000) => {
  try { const r = await fetch(B + "/api/agents/run/" + agent, { method: "POST", headers: { ...sessionHeaders, "content-type": "application/json" }, body: JSON.stringify(payload), signal: AbortSignal.timeout(ms) }); return await r.json(); }
...
(async () => {
  ...
  for (const a of longs) {
    let okN = 0; const det = [];
    for (let i = 0; i < 3; i++) {
      const r = await run("polyglot_translator", { articleId: a.id, targetLanguages: ["de"] });
  ...
  for (const [label, payload] of variants) {
    const t0 = Date.now();
    const r = await run("website_auditor", payload);
