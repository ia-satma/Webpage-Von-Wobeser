...
**How to apply:** don't retry the above paths. **RESOLVED 2026-07-16:** user provided a personal OpenAI API key saved as `AI_INTEGRATIONS_OPENAI_API_KEY`; `AI_INTEGRATIONS_OPENAI_BASE_URL` set to `https://api.openai.com/v1`. All 13 agents are now operational — tested with `gpt-4o-mini` and got a valid response. No code changes were needed.
